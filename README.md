...
```bash
# deps/libtriton_jit。本目录目前不是 git 仓库，所以用 clone 而不是 submodule；
# 若将来 git init 了这个仓库，.gitmodules 已经声明好，改用
# `git submodule update --init --recursive` 即可。
git clone https://github.com/Artlesbol/libtriton_jit.git deps/libtriton_jit
...
`pytest.*` 那些用例跑的是 FlagSparse 的 Python 算子测试，文件留在那边的 checkout 里，
这边只是按 `FLAGSPARSE_PYTHON_SRC` 注册进来（`-DFLAGSPARSE_CTEST_PYTEST=OFF` 可关）。
